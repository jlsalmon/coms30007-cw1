...
$\mathbf{Y}$ is dependent on both $\mathbf{X}$ and $\mathbf{\theta}$, which both contain a level of uncertainty. By marginalising out $\mathbf{X}$, the uncertainty will be filtered down into the marginal distribution of $\mathbf{Y}$. 
\par
Marginalising out $f$ is easier than $\mathbf{X}$, as $\mathbf{X}$ does not depend on anything. 
